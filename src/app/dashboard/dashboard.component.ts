...
export class DashboardComponent implements OnInit {
  ...
  onConnect;
  // s@Input() msgArray: string[] = [];

  userData = this.authService.userObj;
  ...
  constructor( public router: Router, public authService: AuthService, public service: ClientService ) {
  ...
  ngOnInit() {
    ...
    if ( this.userData !== null ) {
      ...
      QB.chat.connect( { userId: this.userData.id, password: this.userData.pass }, ( error ) => {
        if ( error ) {
        ...
        QB.chat.muc.join('60785_599d781ba0eb474ccf844a0e@muc.chat.quickblox.com', (resultStanza) => {
            console.log('QB.chat.muc.join success');
        })
    } );
  ...
  createSession() {
    ...
    QB.createSession( { login: this.userData.login, password: this.userData.pass }, ( error, result ) => {
        if ( error ) {
            console.log( 'error: ', error );
        } else {
